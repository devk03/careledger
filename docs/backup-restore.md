...
Adeno treats recovery as part of patient safety. A backup is only useful after it has been authenticated, hash-checked, and restored successfully.
...
The archive is streamed directly through passphrase-based encryption. Adeno does not create a temporary plaintext tar file.
...
Open **Back up this workspace** from the care dashboard. The owner must enter the current Adeno account password and a separate backup passphrase of at least 12 characters. Adeno then:
...
1. Copy the encrypted file to storage that is not on the Adeno host.
...
- A backup from a newer schema requires compatible application code. Adeno must not auto-downgrade a database.
