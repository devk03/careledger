...
| Grants, revisions and review | Per-member day/source rights apply to every web/API/MCP read and write, including historical versions. Child submissions remain private pending adult approval; stale edits fail. Hidden-day counts, cursors and direct IDs leak nothing. | Managed grant/revision SQL drafts exist. The unmounted v2-envelope reader selects its device from a session binding instead of caller input. `0009`, a browser signer, strict JSON challenge/proof objects, and an unmounted issuance/binding transaction are drafted, but **no managed migration has been applied** and no migrated fictional database has exercised the proof or one-use race. Routes, issuer/write-path binding, re-grant-after-revocation history semantics, transactional browser/API/MCP reads, child review, index visibility and two-family tests remain unresolved. |
...
1. Obtain separate approval to apply the nine registered managed drafts **only** to a
