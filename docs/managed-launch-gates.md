...
| --- | --- | --- |
| Public source and privacy | GitHub repo is publicly reachable, site links to it, the complete pushed history passes `node tools/privacyAudit.mjs` with zero findings, and no real record or secret appears in build/deploy artifacts. | GitHub returned public HTTPS 200 on 2026-09-25 and the source link exists in `web/src/GitHubLink.tsx`; tracked history/index audit passed before this increment. Re-audit the staged release and build/deploy artifacts before promotion. |
| Reproducible builds | Fresh clone builds and starts the documented community edition with no project-owned API key; server/web TypeScript, unit, browser, accessibility and security suites pass on the release commit. | A fresh public feature-branch clone initially failed to start because production omitted `httpx`. After applying the candidate dependency fix to that clone, its community Docker image built and started with AI disabled and empty provider keys on 2026-09-25; `/`, liveness and readiness returned 200, including readiness after a stop/start on the same empty volume. Managed edition is deliberately unavailable; a fresh-clone managed run and release-wide green check are missing. |
| Isolated managed schema | Independently versioned schema applies only to a new fictional DB first; checksums, private file modes, foreign keys, trigger behavior, cross-family rejection, concurrent grant/nonce/revision races and rollback are tested. No automatic conversion of community/real DBs. | With separate approval on 2026-09-26, `0001`–`0010` were applied **once to a new disposable fictional local DB only**. Checksum, private file mode, schema version, integrity and FK checks passed. A rollback-only two-family binding exercise passed cross-family, key, audience, nonce, CSRF, replay, revoked-device, 16-challenge cap and day-intent bound-writer denial checks. Grant/nonce/revision concurrency and complete trigger coverage remain untested. Railway, Hermes, community and real-family DBs were untouched. |
| Individual family identity | Two fictional households can sign up independently; each adult has their own login/device, one-use recipient-bound invitations, session/CSRF/Origin checks, recovery and revocation. A disabled account/device is denied on the next request. | An unmounted managed-v10 identity candidate creates a **frozen** family and pending owner account/membership, issues no signup cookie, and permits Argon2id login only for separately verified active accounts. Current-session managed queries also require verified email. Fictional SQLite tests cover generic denial, family selection, CSRF logout and stale session versions. An unmounted device candidate creates only pending devices; its HTTP router checks cookie-derived identity, configured Origin, CSRF and bounded JSON. Public email verification/resend/expiry, human device approval/activation, managed auth routes, recovery and a deployed shared limiter remain missing. Pending devices have no read/write access. |
| Family-controlled E2EE | Browser encrypts originals, note text, care dates, filenames, derived text and member-appropriate index before upload. Server/DB/logs/backups contain no fictional medical plaintext marker or unwrapped key. Per-day/source/draft keys, verified device enrollment, owner recovery and rotation pass two-family tests and independent security review. Privacy notice states that an operator serving altered web JavaScript could read an unlocked vault; stronger protection needs a separately signed installed client. | Browser crypto and purpose-bound v2 envelopes have focused fictional tests. Ordinary and distinct historical-backfill actions have browser signers, pure server verifiers, cross-runtime fictional signature vectors and unmounted transactional writers. `0006`–`0008` exist in the one fictional schema, but neither writer has run against managed SQLite; historical backfill approval/UI, read-time authorization, complete key hierarchy, client witness and independent deployed-flow review are missing. The web-delivery threat is documented in `docs/e2ee-managed.md`, not eliminated. |
...
| Record intake and daily timeline | Batch PDF/photo/note intake preserves originals and upload times; care/document/report dates remain separate. One populated day can hold several files. Undated, planned, occurred and corrected entries are source-linked and versioned. Late earlier-day material appears in a backward read without a cached AI consensus. | Community UI and trusted pilot cover pieces; an unmounted encrypted draft preparer and client-side timeline projector exist. Additive `0005` tables exist only in the disposable fictional schema, and their source/draft runtime behavior has not been tested; no draft-intent issuer, non-day runtime, persisted/authorized purpose-bound key envelope, adult review path or end-to-end browser flow exists. |
...
1. The separately approved 2026-09-26 application of the ten registered
   managed drafts to **one new private fictional local database** is complete.
   Continue two-family trigger and concurrency tests there; do not apply a
   migration to the community, Hermes, Railway or real-family database without
   separate approval. Keep the historical-backfill writer unmounted until its
   distinct signed action and transactional behavior pass fictional database tests.
...
The `0010` SQL file's frozen “has not been executed” comment described its
creation-time state. Its checksum is immutable; the factual application status
is the fictional-only record above, not that historical comment.
