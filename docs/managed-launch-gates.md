...
| Individual family identity | Two fictional households can sign up independently; each adult has their own login/device, one-use recipient-bound invitations, session/CSRF/Origin checks, recovery and revocation. A disabled account/device is denied on the next request. | Local trusted TypeScript pilot has some individual-account behavior. An unmounted managed-v10 candidate now issues a session-bound enrollment challenge, checks X25519 private-key knowledge and an Ed25519 signature, consumes once and creates a **pending** device. Fictional SQLite and Chromium tests pass. Human/family approval and activation, managed signup/session routes, recovery and operating rate limits are missing; pending devices have no read/write access. |
