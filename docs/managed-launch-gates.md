...
| Grants, revisions and review | Per-member day/source rights apply to every web/API/MCP read and write, including historical versions. Child submissions remain private pending adult approval; stale edits fail. Hidden-day counts, cursors and direct IDs leak nothing. | Managed grant/revision SQL drafts exist. The unmounted v2-envelope reader, both issuer writers and the day-upload ledger now select or require the session's bound device. `0009`, a browser signer, strict JSON challenge/proof objects, and an unmounted issuance/binding transaction are drafted, but **no managed migration has been applied** and no migrated fictional database has exercised these joins or the one-use race. Other managed write paths and SQL insert triggers still need binding checks; routes, re-grant-after-revocation history semantics, transactional browser/API/MCP reads, child review, index visibility and two-family tests remain unresolved. |
