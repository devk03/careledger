...
| Individual family identity | Two fictional households can sign up independently; each adult has their own login/device, one-use recipient-bound invitations, session/CSRF/Origin checks, recovery and revocation. A disabled account/device is denied on the next request. | An unmounted managed-v10 identity candidate creates a **frozen** family and pending owner account/membership, issues no signup cookie, and permits Argon2id login only for separately verified active accounts. Its separate unmounted login/session/logout HTTP router passed fictional HTTP+SQLite tests with a Secure HttpOnly cookie, fresh CSRF, revocation and generic denial. Current-session managed queries require verified email. Device enrollment remains pending-only, with its own unmounted HTTP router. Public email verification/resend/expiry, human device approval/activation, a mounted managed composition, recovery and a deployed shared limiter remain missing. |
