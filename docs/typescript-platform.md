...
Target architecture, 2026-09-23. This is not a claim that the hosted family product or the TypeScript server is production-ready. The [day-by-day product contract](treatment-timeline-product.md) controls the data model. The existing Python app remains operational during migration.
...
The visible unit is a **sparse calendar-day node**, keyed by `(care_profile_id, YYYY-MM-DD)`. It can be derived from approved file placements and family notes rather than stored as an empty day row. A node can link multiple original files and notes; a file may link to more than one day if it discusses multiple dates. No material for a date means “nothing recorded in Adeno for that day,” not “no care occurred.” Unknown or conflicting care dates remain in a review queue; the upload timestamp is never substituted for a care day. See the [additive storage proposal](day-storage-design.md).
...
The first TypeScript package in `server/` has a pure backward-history service, injected Express read routes, and in-memory-tested MCP tools for sparse days and bounded approved source-page text. A fictional vertical-slice test compares HTTP and MCP responses before and after a later upload is attached to an earlier care day, and checks cross-family denial. A preliminary upload-admission helper validates size, signature, claimed MIME type, display name and hash without storing bytes. It does **not** replace the current quarantine, malware scan or isolated PDF/image inspection. The package has no production authentication, persistence adapter, upload route, public MCP transport or deployment entrypoint. Those omissions are intentional fail-closed boundaries, not finished features.

Production authorization must preserve the current secure cookie, hashed session tokens, CSRF binding, Origin checks, audit behavior and per-document access checks until replaced by proven TypeScript equivalents. Never accept a model-supplied household/user ID as authority. Cross-family reads should return the same not-found response as an unknown profile. MCP authorization is a separate, short-lived, scoped connection grant; linking a client alone must not bypass family-controlled encryption.
